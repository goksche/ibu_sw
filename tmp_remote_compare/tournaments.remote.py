...
from app.core.dependencies import require_user_or_admin, require_viewer_or_above, get_current_user, require_admin
...
from app.services.ko_propagation import can_enter_ko_result, save_ko_result_and_propagate
...
async def update_tournament(
    ...
    if tournament.status == TournamentStatus.RUNNING:
        locked_changes = sorted(field for field in update_data.keys() if field in RUNNING_LOCKED_FIELDS)
        if locked_changes:
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
                detail=(
                    "Turnier läuft bereits. Grundeinstellungen können nicht mehr geändert werden. "
                    "Erlaubt sind nur Turnierspiel-Aktionen wie Resultate und Auslosungen."
                ),
            )
...
async def simulate_tournament_phase(
    ...
    max_round_row = (
        db.query(KnockoutMatch.round)
        .filter(KnockoutMatch.tournament_id == tournament_id, KnockoutMatch.round >= 1)
        .order_by(KnockoutMatch.round.desc())
        .first()
    )
    ...
    for round_no in range(1, max_round + 1):
        ...
        for match in round_matches:
            ...
            # Persist scores on the simulated match so UI and next-round checks
            # can treat this round as completed.
            match.score1 = score1
            match.score2 = score2
            db.flush()
    ...
    bronze_matches = (
        db.query(KnockoutMatch)
        .filter(KnockoutMatch.tournament_id == tournament_id, KnockoutMatch.round == 99)
        .order_by(KnockoutMatch.match_no)
        .all()
    )
...
async def generate_ko_bracket_matches(
    ...
    if tournament.mode == TournamentMode.KNOCKOUT and not groups:
        ...
        elif tournament.ko_structure and tournament.ko_structure.value == 'page_playoff':
        ...
        if tournament.ko_structure and tournament.ko_structure.value == 'consolation_bracket':
            ...
            elif tournament.ko_structure.value == 'page_playoff':
                from app.services.ko_bracket import generate_page_playoff_bracket
                ko_matches = generate_page_playoff_bracket(
                    participant_ids=qualified_participants,
                    draw_method=mode if mode in ('full_random', 'pot_system', 'overall_seeding') else 'overall_seeding',
                    rng_seed=tournament.ko_random_seed
                )
...
async def duplicate_tournament(
    ...
    new_tournament_data = {
        ...
        'play_time_start': original.play_time_start,
        'play_time_end': original.play_time_end,
        'match_duration_minutes': original.match_duration_minutes,
        'mode': original.mode,
