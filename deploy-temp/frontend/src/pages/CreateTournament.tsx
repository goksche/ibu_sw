...
import { Tournament, Participant, LeagueVariant } from '../types';
...
export default function CreateTournament() {
  ...
  useEffect(() => {
    const loadTemplates = async () => {
      // #region agent log
      fetch('http://127.0.0.1:7242/ingest/37a2199e-e139-4caa-bc6d-01e215fbd275',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'CreateTournament.tsx:60',message:'loadTemplates entry',data:{},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'ALL'})}).catch(()=>{});
      // #endregion
      try {
        setLoadingTemplates(true);
        // #region agent log
        fetch('http://127.0.0.1:7242/ingest/37a2199e-e139-4caa-bc6d-01e215fbd275',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'CreateTournament.tsx:63',message:'Before getTemplates call',data:{},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'ALL'})}).catch(()=>{});
        // #endregion
        const templateList = await tournamentService.getTemplates();
        // #region agent log
        fetch('http://127.0.0.1:7242/ingest/37a2199e-e139-4caa-bc6d-01e215fbd275',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'CreateTournament.tsx:65',message:'getTemplates success',data:{templateCount:templateList?.length||0},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'ALL'})}).catch(()=>{});
        // #endregion
        setTemplates(templateList);
      } catch (err: any) {
        // #region agent log
        const errorData = {errorType:err?.constructor?.name,message:err?.message,responseStatus:err?.response?.status,responseData:err?.response?.data,code:err?.code,isAxiosError:err?.isAxiosError,configUrl:err?.config?.url,configBaseURL:err?.config?.baseURL};
        // Debug-Log als Warnung, nicht als Fehler
        console.warn('[DEBUG] Failed to load templates - Full error:', errorData);
        fetch('http://127.0.0.1:7242/ingest/37a2199e-e139-4caa-bc6d-01e215fbd275',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'CreateTournament.tsx:66',message:'getTemplates error',data:errorData,timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'ALL'})}).catch(()=>{});
        // #endregion
  ...
  const handleTemplateSelect = async (templateId: number) => {
    try {
      ...
      setFormData({
        ...
        ko_distribution: template.ko_distribution as 'cross' | 'draw' || 'cross',
