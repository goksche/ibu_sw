...
export type KOStructure =
  ...
  | 'single_elimination_with_ranking' // API: Platzierungsspiele (nicht nur Bronze)
  | 'consolation_bracket'
  | 'double_elimination'
  | 'triple_elimination'
  | 'aggregate_ko'
...
/** Wie Gruppen gefüllt werden (`schemas/tournament.py` Pattern) */
export type GroupDistribution = 'random' | 'seeded' | 'manual';

/** Spielfeld-/Platz-Zuweisung in der Gruppenphase */
export type SpielfeldAssignmentMode = 'random' | 'group_fixed' | 'group_random';
...
export interface Tournament {
  ...
  /**
   * Legacy-Spiegel zur KO-Auslosung (historisches Feld).
   * Neue Saves: `ko_draw_method` ist führend — nicht parallel zu diesem Feld senden (siehe Liga-Erstellung).
   */
  ko_distribution: KODrawMode | null;
  ...
  spielfeld_assignment_mode?: SpielfeldAssignmentMode | null;
  created_at: string;
  updated_at: string;
