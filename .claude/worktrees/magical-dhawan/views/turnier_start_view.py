# views/turnier_start_view.py
# v0.9.6.1 – Turnier starten: aktuelle Turniere, Teilnehmerzuweisung, Gruppenbildung
...
from PyQt6.QtWidgets import (
    ...
    QInputDialog, QLineEdit, QVBoxLayout as QV
)

# WICHTIG: entspricht deinem Projekt (database/models.py)
from database.models import (
    fetch_turniere,                # -> List[Tuple[id, name, datum, modus, meisterschaft]]
    fetch_teilnehmer,              # -> List[Tuple[id, name, spitzname]]
    fetch_turnier_teilnehmer,      # -> List[Tuple[id, name]]
    set_turnier_teilnehmer,        # (turnier_id, [teilnehmer_ids])
    has_grouping,                  # (turnier_id) -> bool
    fetch_grouping,                # (turnier_id) -> Dict["A"|"B"|..., List[Tuple[id,name,spitz]]]
    save_grouping,                 # (turnier_id, List[Tuple[gruppenname, List[teilnehmer_ids]]])
    clear_grouping                 # (turnier_id) -> None
)

GROUP_MIN = 2
GROUP_MAX = 8
DELETE_PASSWORD = "6460"  # Schutz beim Ueberschreiben einer bestehenden Gruppierung


class TurnierStartView(QWidget):
    """
    Verbesserungen v0.9.6.1:
    - __init__(parent=None) kompatibel zu main_window._safe_instantiate(...)
    - IDs immer als itemData(UserRole) in Combos/Listen
    - Reload beim showEvent + manuell + Auto-Refresh (5s)
    - Auswahlbeibehaltung beim Reload
    """
    def __init__(self, parent: Optional[QWidget] = None):
        super().__init__(parent)
        self.setObjectName("TurnierStartView")

        self._staged_groups: List[List[int]] = []
        ...
        self.btn_add = QPushButton("→ Hinzufuegen"); self.btn_add.clicked.connect(self._add_selected)
        mid_btns.addWidget(self.btn_add)
        self.btn_remove = QPushButton("← Entfernen"); self.btn_remove.clicked.connect(self._remove_selected)
        mid_btns.addWidget(self.btn_remove)
        self.btn_add_all = QPushButton("≫ Alle hinzufuegen"); self.btn_add_all.clicked.connect(self._add_all)
        mid_btns.addWidget(self.btn_add_all)
        self.btn_remove_all = QPushButton("≪ Alle entfernen"); self.btn_remove_all.clicked.connect(self._remove_all)
        ...
        self.btn_autosplit = QPushButton("Auto verteilen"); self.btn_autosplit.clicked.connect(self._auto_split)
        row_groups.addWidget(self.btn_autosplit)
        self.btn_save_groups = QPushButton("Gruppierung speichern"); self.btn_save_groups.clicked.connect(self._save_groups)
        row_groups.addWidget(self.btn_save_groups)
        self.btn_clear_groups = QPushButton("Gruppierung loeschen"); self.btn_clear_groups.clicked.connect(self._clear_groups)
        ...
        # Auto-Refresh der Turnierliste alle 5 Sekunden
    ...
    # Reload bei Tab-Anzeige
    def showEvent(self, event):
        super().showEvent(event)
        self._reload_turniere_keep_selection()

    # ---------- Laden Turniere/Listen ----------
    def _current_turnier_id(self) -> Optional[int]:
        return self.cbo_turnier.currentData(Qt.ItemDataRole.UserRole)

    def _set_combo_by_id(self, wanted_id: Optional[int]) -> None:
        if wanted_id is None:
            return
        for i in range(self.cbo_turnier.count()):
            if self.cbo_turnier.itemData(i, Qt.ItemDataRole.UserRole) == wanted_id:
    ...
    def _reload_turniere_keep_selection(self):
        ...
        items = fetch_turniere()  # [(id, name, datum, modus, meisterschaft)]
    ...
    def _load_participants_lists(self):
        ...
        all_tn = fetch_teilnehmer()            # [(id, name, spitzname)]
        in_tn = fetch_turnier_teilnehmer(tid)  # [(id, name)]
        in_ids = {int(r[0]) for r in in_tn}

        for pid, name, nick in all_tn:
            text = f"{name}" + (f" ({nick})" if nick else "")
            item = QListWidgetItem(text)
            item.setData(Qt.ItemDataRole.UserRole, int(pid))
            if int(pid) in in_ids:
    ...
    def _add_selected(self):
        ...
        self._staged_groups = []; self._staged_group_names = []
        self._load_group_preview()
    ...
    def _load_group_preview(self):
        ...
        grouping = fetch_grouping(tid)  # gespeicherte Gruppierung?
        if grouping:
        ...
        # sonst Vorschau anzeigen (falls vorhanden)
        if self._staged_groups:
    ...
    def _save_groups(self):
        ...
        if not self._staged_groups or not self._staged_group_names:
            QMessageBox.warning(self, "Fehler", "Keine Gruppierung in der Vorschau. Bitte zuerst „Auto verteilen“.")
            return
