...
export const tournamentService = {
  ...
  async getTemplates(): Promise<Tournament[]> {
    // #region agent log
    fetch('http://127.0.0.1:7242/ingest/37a2199e-e139-4caa-bc6d-01e215fbd275',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'tournamentService.ts:74',message:'getTemplates entry',data:{endpoint:'/tournaments/templates'},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'ALL'})}).catch(()=>{});
    // #endregion
    try {
      const response = await api.get<Tournament[]>('/tournaments/templates');
      // #region agent log
      fetch('http://127.0.0.1:7242/ingest/37a2199e-e139-4caa-bc6d-01e215fbd275',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'tournamentService.ts:76',message:'getTemplates API success',data:{status:response?.status,dataLength:response?.data?.length},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'ALL'})}).catch(()=>{});
      // #endregion
      return response.data;
    } catch (err: any) {
      // #region agent log
      ...
      console.error('[DEBUG] tournamentService.getTemplates error:', errorData);
      fetch('http://127.0.0.1:7242/ingest/37a2199e-e139-4caa-bc6d-01e215fbd275',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'tournamentService.ts:78',message:'getTemplates API error',data:errorData,timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'ALL'})}).catch(()=>{});
      // #endregion
      throw err;
    }
  },
